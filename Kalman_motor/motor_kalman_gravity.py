...
observation_data = motor_gravity.yn_list + rand_loc
#システム行列
system_mat = motor_gravity.mat_1
system_mat[1,0] = h_kalman
#制御入力
u = motor_gravity.delta_T
#制御行列
control_mat = motor_gravity.mat_3
control_mat[0,0] = h_kalman / motor_gravity.m
#単位行列
...
for i in range(n):
    if t_s >= tc:
        ...
        L = np.matrix([[motor_gravity.vn_list[i]],[observation_data[i]]])
        Z = observation_mat * L
...
ax1.plot(motor_gravity.T_m,motor_gravity.vn_list,label = "True velocity")
ax1.grid()
ax1.set_xlabel("time")
ax1.set_ylabel("V[m/s]")

ax2.plot(T,mu_y_list,label = "estimate location")
ax2.plot(motor_gravity.T_m,motor_gravity.yn_list,label = "True location")
ax2.grid()
ax2.set_xlabel("time")
ax2.set_ylabel("location[m]")

ax3.plot(motor_gravity.T_m,observation_data,label = "Observedata")
ax3.grid()
ax3.set_xlabel("time")
